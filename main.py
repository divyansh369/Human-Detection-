        ...
        def detectByPathImage(path):
            ...
            def img_enumeration_plot():
                ...
                plt.get_current_fig_manager().canvas.set_window_title(
                    "Plot for Image")  # plt.manager.set_window_title("Plot for Image")
                plt.show()
            ...
            def img_accuracy_plot():
                ...
                plt.plot(framex1, avg_acc1_list, label="Avg. Accuracy",
                         color="green", marker='o', markerfacecolor='blue')
                plt.plot(framex1, max_avg_acc1_list, label="Max. Avg. Accuracy",
                         linestyle='dashed', color='fuchsia')
                plt.xlabel('Time (sec)')
            ...
            def img_gen_report():
                ...
                if (max_count1 > 25):
                    pdf.text(
                        26, 220, "Max. Human Detected is greater than MAX LIMIT.")
                    pdf.text(70, 235, "Region is Crowded.")
                else:
                    pdf.text(
                        26, 220, "Max. Human Detected is in range of MAX LIMIT.")
                    pdf.text(65, 235, "Region is not Crowded.")

                pdf.output('Crowd_Report.pdf')
                mbox.showinfo(
                    "Status", "Report Generated and Saved Successfully.", parent=windowi)
            ...
            for i in range(len(boxes)):
                ...
                if classes[i] == 1 and scores[i] > threshold:
                    ...
                    cv2.putText(img, f'P{person, round(scores[i], 2)}', (
                        box[1] - 30, box[0] - 8), cv2.FONT_HERSHEY_COMPLEX, 0.5, (0, 0, 255), 1)  # (75,0,130),
                    acc += scores[i]
            ...
            Button(windowi, text="Avg. Accuracy\nPlot", command=img_accuracy_plot, cursor="hand2", font=(
                "Arial", 20), bg="orange", fg="blue").place(x=700, y=530)
            Button(windowi, text="Generate  Crowd  Report", command=img_gen_report, cursor="hand2", font=(
                "Arial", 20), bg="light gray", fg="blue").place(x=325, y=550)
    ...
    def video_option():
        ...
        def detectByPathVideo(path, writer):
            ...
            def vid_enumeration_plot():
                ...
                ax = plt.axes()
                ax.set_facecolor("yellow")
                plt.plot(framex2, county2, label="Human Count",
                         color="green", marker='o', markerfacecolor='blue')
            ...
            def vid_gen_report():
                ...
                pdf.output('Crowd_Report.pdf')
                mbox.showinfo(
                    "Status", "Report Generated and Saved Successfully.", parent=windowv)
                ...
                if (check == True):
                    ...
                    for i in range(len(boxes)):
                        if classes[i] == 1 and scores[i] > threshold:
                            ...
                            person += 1
                            cv2.rectangle(
                                img, (box[1], box[0]), (box[3], box[2]), (255, 0, 0), 2)
                            cv2.putText(img, f'P{person, round(scores[i],2)}', (
                                box[1]-30, box[0]-8), cv2.FONT_HERSHEY_COMPLEX, 0.5, (0, 0, 255), 1)  # (75,0,130),
                            acc += scores[i]
            ...
            Button(windowv, text="Enumeration\nPlot", command=vid_enumeration_plot,
                   cursor="hand2", font=("Arial", 20), bg="orange", fg="blue").place(x=100, y=530)
            Button(windowv, text="Avg. Accuracy\nPlot", command=vid_accuracy_plot, cursor="hand2", font=(
                "Arial", 20), bg="orange", fg="blue").place(x=700, y=530)
    ...
    def camera_option():
        ...
        def detectByCamera(writer):
            ...
            def cam_gen_report():
                ...
                pdf.output('Crowd_Report.pdf')
                mbox.showinfo(
                    "Status", "Report Generated and Saved Successfully.", parent=windowc)
            ...
            Button(windowc, text="Enumeration\nPlot", command=cam_enumeration_plot,
                   cursor="hand2", font=("Arial", 20), bg="orange", fg="blue").place(x=100, y=530)
            Button(windowc, text="Avg. Accuracy\nPlot", command=cam_accuracy_plot, cursor="hand2", font=(
                "Arial", 20), bg="orange", fg="blue").place(x=700, y=530)
